...
export function V2Progress({ runId }: { runId: string }) {
  ...
  return <>
    <div className="v2-page-heading"><Link className="v2-back" href="/research">← New research</Link><p className="v2-eyebrow">RESEARCH IN PROGRESS</p><h1>{run.product_name}</h1><p>Started {runDate(run.created_at)} UTC · Elapsed {elapsed(run.started_at || run.created_at, now)}</p></div>
    ...
    {run.status === "failed" && <div className="v2-alert v2-alert-error" role="alert">{run.failure?.message || "The analysis could not be completed."} <Link href="/research">Try a more specific product name</Link>.</div>}
