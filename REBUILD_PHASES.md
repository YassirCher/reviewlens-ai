...
| 3 | OpenRouter gateway and LLMOps accounting core | complete | Completed 2026-09-15: `python scripts/check_context.py`; `git diff --check`; `cd backend && .venv\Scripts\python.exe -m pytest tests -q` (31 passed, 29 Docker-gated tests skipped); `python scripts/check_phase3.py` (60 isolated PostgreSQL/Redis/Celery and mocked OpenRouter tests, empty-to-head migration, downgrade to `20260915_0002`, re-upgrade, full-stack health, V1 smoke, and mocked chat/embedding fixtures). The checker used generated fake upstream credentials, made no paid calls, preserved the frontend dirty inventory, and confirmed `git ls-files context` is empty. |
| 4 | Reconstructable context graph and retrieval | complete | Completed 2026-09-16: `python scripts/check_context.py`; `git diff --check`; `cd backend && .venv\Scripts\python.exe -m pytest tests -q` (39 passed, 36 Docker-gated tests skipped); `python scripts/check_phase4.py` (75 isolated PostgreSQL/Redis/Celery/Neo4j/Qdrant tests, empty-to-head migration, downgrade to `20260915_0003`, re-upgrade, immutable graph/manifests, atomic rollback reconciliation, real projection rebuilds, deterministic/degraded retrieval fixtures, full-stack health, and V1 smoke). The checker used generated test credentials and deterministic local vectors, made no paid calls, preserved the frontend dirty inventory, and confirmed `git ls-files context` is empty. |
| 5 | Typed YouTube research toolchain | complete | Completed 2026-09-16: `python scripts/check_context.py`; `git diff --check`; `cd backend && .venv\Scripts\python.exe -m pytest tests -q` (49 passed, 43 Docker-gated tests skipped); `python scripts/check_phase5.py` (92 isolated PostgreSQL/Redis/Celery/Neo4j/Qdrant tests, empty-to-head migration, downgrade to `20260915_0004`, re-upgrade, idempotent tool seeding, mocked YouTube ingestion, invocation/quota constraints, full-stack health, and V1 smoke). Complete, missing-transcript, comments-off, and partial fixtures passed; the checker used generated credentials, made no live or paid calls, removed its environment/volumes, preserved the byte-level frontend inventory, and confirmed `git ls-files context` is empty. |
| 6 | Bounded multi-agent analysis workflow | complete | Completed 2026-09-16: `python scripts/check_context.py`; `git diff --check`; `cd backend && .venv\Scripts\python.exe -m pytest tests -q` (68 passed, 44 Docker-gated tests skipped); `python scripts/check_phase6.py` (112 containerized tests, empty-to-head migration, downgrade to `20260916_0005`, re-upgrade, full-stack health and V1 smoke). All seven mocked analysis fixtures passed, including comments, partial coverage, exactly one retry correction, audit correction/failure, and cancellation. The checker verified sanitized logs, generated fake credentials, no live or paid calls, unchanged frontend bytes, cleanup of generated test environment/volumes, and `git ls-files context` empty. |
| 7 | Public V2 API and report lifecycle | complete | Completed 2026-09-16: `python scripts/check_context.py`; `git diff --check`; `cd backend && .venv\Scripts\python.exe -m pytest tests -q` (75 passed, 48 Docker-gated tests skipped); `python scripts/check_phase7.py` (123 containerized tests, empty-to-head migration, downgrade to `20260916_0006`, re-upgrade, full Compose health, V1 smoke, mocked public report/revocation/SSE and concurrent-idempotency checks, partial publication, and failed-audit withholding). The checker used generated fake credentials and local OpenRouter/YouTube mocks, made no live or paid calls, removed generated environment/volumes, preserved frontend bytes, and confirmed `git ls-files context` is empty. |
| 8 | Public research experience | complete | Completed 2026-09-17: `python scripts/check_context.py`; `git diff --check`; `cd backend && .venv\Scripts\python.exe -m pytest tests -q` (80 passed, 49 Docker-gated tests skipped); `cd frontend && npm.cmd run lint`; `cd frontend && npx.cmd tsc --noEmit`; `cd frontend && npm.cmd run test:unit` (5 passed); `python scripts/check_phase1.py` through `python scripts/check_phase7.py` (each passed its 129-test isolated container suite, migration cycle, full-stack health, V1 smoke, and phase fixtures); `python scripts/check_phase8.py` (repeated Phase 7 migration/API gate, healthy generated-credential Compose stack, one production browser submission-to-report/evidence journey, and 16 mocked browser regressions covering ownership, replay, failures, keyboard access, zoom, reduced motion, and 375/768/1024/1440px layouts). Local OpenRouter/YouTube mocks made no live or paid calls; pre-existing `tsconfig.json`, `AGENTS.md`, `CLAUDE.md`, and `next-env.d.ts` hashes remained unchanged, while the pre-existing package lock was extended for Playwright; generated environments/volumes were removed, and `git ls-files context` was empty. |
| 9 | Admin LLMOps control plane | complete | Completed 2026-09-19: `backend\.venv\Scripts\python.exe scripts/check_phase9.py` passed the complete prior Phase 7/8 regression chain, 139 containerized backend tests, empty-to-head and Phase 9 downgrade/re-upgrade migrations, one real-stack public browser journey, 16 public browser regressions, six Phase 9 PostgreSQL/Celery/mocked-OpenRouter integration tests, and three admin browser journeys. The production frontend build and TypeScript checks passed for every reported static and dynamic route; local provider mocks made no live or paid calls. Coverage includes immutable published versions, optimistic conflicts, active-run snapshot isolation, evaluated publication gates and shared-budget races, aggregate reconciliation, authentication/CSRF, graph isolation, refresh failures, recovery actions, audit redaction, keyboard/reader alternatives, reduced motion, 200% zoom, and 375/768/1024/1440px layouts. Generated environments/volumes were removed, pre-existing frontend files were preserved, and `context/` remained outside Git. |
| 10 | Whole-system hardening and acceptance | complete | Completed 2026-09-20: `backend\.venv\Scripts\python.exe scripts/check_phase10.py --static-only` passed Ruff, targeted mypy, 92 local tests (55 stack-gated), pip and npm vulnerability audits, exact npm install, frontend lint, five unit tests, Next.js 16.3.3 production build, context/secret/diff checks; `--stack-only` passed 147 containerized backend tests, empty-to-head and Phase 9 downgrade/re-upgrade migrations, PostgreSQL backup/restore, V1 smoke, real public/admin browser journeys with Axe, Redis/PostgreSQL outage recovery, generated-secret/live-endpoint scans, and cleanup; `--browser-only` passed 17 public and four admin mocked journeys covering keyboard/live regions, reduced motion, 200% zoom, hostile content, and 375/768/1024/1440px layouts. Local OpenRouter/YouTube mocks made no live or paid calls, and the four user-owned frontend files remained byte-preserved. |
| 11 | Compatibility, V2 cutover, and operational proof | complete | Completed 2026-09-20: `backend\.venv\Scripts\python.exe scripts/check_phase11.py --static-only` passed Ruff, targeted mypy, 97 local tests (56 stack-gated), pip/npm audits with no vulnerabilities, frontend lint/unit/production build, context/secret/diff checks, and protected-file hashes; `--stack-only` passed 153 containerized backend tests, empty-to-head plus `0009` downgrade/re-upgrade, backup/restore, real V2 and legacy-adapter runs, compatibility telemetry, a passing shortened test observation, root V1 rollback/V2 restore, Redis/PostgreSQL recovery, production browser smoke, generated-secret/live-endpoint scans, and cleanup; `--browser-only` passed 17 public and four admin accessibility/responsive journeys plus runtime root rollback. Mock history proved every Phase 11 inference used exactly `deepseek/deepseek-v4-flash` and no live or paid call occurred. The required 24-hour non-test production observation remains the explicit Phase 12 authorization gate. |
| 12 | Legacy retirement and final V2 conformance | complete | Completed 2026-09-20 for the working project: `scripts/check_phase12.py --static-only` passed Ruff, targeted mypy, 93 local tests (57 stack-gated), pip/npm audits, frontend lint/unit/build, context/secret/diff checks; `--stack-only` passed 150 containerized backend tests, empty-to-head plus `0009` downgrade/re-upgrade, backup/restore, three real-stack browser journeys, both approved mocked DeepSeek Flash models with full attribution, Redis/PostgreSQL recovery, live-endpoint/log scans, and cleanup; mocked browser acceptance passed 21 journeys. Production retirement evidence remains a separate deployment authorization checked with `--evidence-only` when deploying over a real Phase 11 environment. |
...
**Goal:** Introduce a separate calm, evidence-led V2 research experience at `/research` that makes product submission, live task progress, persistent reports, source inspection, token totals, and a simplified accessible evidence map clear on every target viewport. Keep the legacy `/` screen until the Phase 11 cutover.
